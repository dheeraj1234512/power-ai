...
def init_session_state():
    defaults = {
        "logged_in": False, "username": "", "is_guest": False,
        "messages": [], "store": {}, "current_chat_id": datetime.now().strftime(TIMESTAMP_ID),
        "all_chats": {}, "active_tab": "login", "reg_success": False, "dark_mode": True,
    }
...
# ===== HELPER FUNCTIONS =====
def get_timestamp_id(): return datetime.now().strftime(TIMESTAMP_ID)
def get_timestamp_datetime(): return datetime.now().strftime(TIMESTAMP_DATETIME)
def get_timestamp_display(): return datetime.now().strftime(TIMESTAMP_DISPLAY)
...
def get_theme_colors():
    if st.session_state.dark_mode:
        return {"bg": "#0b0f14", "panel": "#111823", "text": "#e8eef5", "muted": "#93a4b5", "soft": "rgba(255,255,255,0.04)", "border": "rgba(255,255,255,0.08)"}
    return {"bg": "#ffffff", "panel": "#f0f2f5", "text": "#1a1a1a", "muted": "#666666", "soft": "rgba(0,0,0,0.04)", "border": "rgba(0,0,0,0.1)"}

def get_css_styles(dark_mode=True):
    theme = get_theme_colors()
    return f"""
@import url('https://fonts.googleapis.com/css2?family=Inter:wght@300;400;500;600&display=swap');
:root {{ --bg: {theme["bg"]}; --panel: {theme["panel"]}; --soft: {theme["soft"]}; --border: {theme["border"]}; --text: {theme["text"]}; --muted: {theme["muted"]}; --accent: #4f7cff; --radius: 14px; }}
...
.chat-item {{ padding: 10px 12px; border-radius: var(--radius); border: 1px solid transparent; color: var(--muted); transition: 0.2s ease; cursor: pointer; }}
...
.stTextInput input, .stChatInput textarea {{ background: var(--panel); border-radius: var(--radius); color: var(--text); padding: 10px 12px; }}
.stTextInput input:focus, .stChatInput textarea:focus {{ border-color: var(--accent); box-shadow: none !important; outline: none !important; }}
.stButton button {{ background: var(--accent); color: white; border-radius: 10px; border: none; font-weight: 500; transition: 0.2s ease; }}
...
def register_user(username, password):
    ...
    if users_sheet:
        try:
            ...
            users_sheet.append_row([username, hash_password(password), get_timestamp_datetime()])
...
def save_chat(username, question, answer, chat_id):
    ...
    if chat_sheet:
        try:
            chat_sheet.append_row([get_timestamp_datetime(), question, answer, username, chat_id])
        except:
            pass
...
if not st.session_state.logged_in:
    ...
    with col2:
        if st.session_state.reg_success: st.success("✅ Account Created! Login now!")
        ...
        with tab1:
            ...
            if st.button("Login ⚡", use_container_width=True, key="login_btn"):
                ...
                else: st.warning("⚠️ Fill in all fields!")
        ...
        with tab2:
            ...
            if st.button("Register ⚡", use_container_width=True, key="reg_btn"):
                if new_user and new_pass and confirm_pass:
                    if new_pass != confirm_pass: st.error("❌ Passwords don't match!")
                    else:
...
# ===== CHAT PAGE =====
                ...
                if chat_msgs:
                    ...
                    with col2:
                        if st.button("🗑️", key=f"del_{chat_id}"):
                            ...
                            st.rerun()
        st.divider()
        if st.session_state.is_guest: st.markdown("👤 **Guest Mode**")
        else: st.markdown(f"👤 **{st.session_state.username}**")
    ...
    # AI Setup (No Web Search - Safe Mode)
    def init_chatbot():
        llm = ChatGroq(model=MODEL_NAME, temperature=TEMPERATURE)
        prompt = ChatPromptTemplate.from_messages([
            ("system", f"""
    You are **Power AI** — an advanced, intelligent, and highly reliable AI assistant.
    ...
    # Show Messages using Native Streamlit
