...
with st.expander("🔐 Admin Panel"):
    ...
    if admin_pass == "power2024":  # apna password rakho
        st.success("✅ Welcome Admin!")
        
        if st.button("🔄 Data Refresh Karo"):
            sheet = get_sheet()
            if sheet:
                ...
                if data:
                    ...
                    col2.metric("👥 Users", df["Session ID"].nunique())
                    col3.metric("📅 Aaj", len(df[df["Timestamp"].str.startswith(datetime.now().strftime("%Y-%m-%d"))]))
